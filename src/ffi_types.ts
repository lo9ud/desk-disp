...
/**
 * Parsed once in `main`, then injected into every webview as a frozen global
 * before any frontend code runs - see `prepare_webview` in `lib.rs`. Adding a
 * flag here makes it readable frontend-side with no further wiring.
 */
export type Args = { 
/**
 ...
 *
 * Media type not included; frontend can infer it from the first few bytes of the b64 data.
 * 
 * @see {@link import('./src/utils/image.ts').imageDataUrl} for the magic bytes used to sniff the type.
 */
